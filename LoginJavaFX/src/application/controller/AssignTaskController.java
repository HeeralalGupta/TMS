...
public class AssignTaskController implements Initializable {
	...
	@FXML
	private TableColumn<DataObject, String> assignColumn;
	@FXML
	...
	public void initialezeTable() {
		...
		assignColumn.setCellValueFactory(new PropertyValueFactory<>("assignDate"));
		...
		Callback<TableColumn<DataObject, Void>, TableCell<DataObject, Void>> cellFactory = new Callback<TableColumn<DataObject, Void>, TableCell<DataObject, Void>>() {
			...
			public TableCell<DataObject, Void> call(final TableColumn<DataObject, Void> param) {
				final TableCell<DataObject, Void> cell = new TableCell<DataObject, Void>() {
					...
					Image viewTask= new Image("image/view.png");
					Image delete = new Image("image/delete.png");

					ImageView taskView = new ImageView(viewTask);
					...
					HBox box = new HBox();
					{
						HBox.setMargin(viewBtn, new Insets(0, 10, 0, 0));
						HBox.setMargin(deleteBtn, new Insets(0, 10, 0, 0));
						box.getChildren().addAll(viewBtn, deleteBtn);
						taskView.setFitWidth(20); // Set your preferred width
						taskView.setFitHeight(20);
						viewBtn.setGraphic(taskView);
						viewBtn.setOnAction((ActionEvent event) -> {
							...
							try {
								...
								FXMLLoader loader = new FXMLLoader(
										getClass().getResource("/application/fxml/viewAssignTasks.fxml"));
								root = loader.load();
								ViewAssignTaskController addTask = loader.getController();
								addTask.data(data.getId(),data.getTaskName(),data.getEstimateDate(),
										data.getStatus(),data.getAssignTo(),data.getCurrentOwner(),
										data.getOwner(),data.getAssignDate(),data.getPriority(),data.getDescription(),
										data.getAssignId());
	...
	private void fetchDataFromDatabase(int page) {
		try {
		    ...
		    if(resultSet.getString("assignTo").equals("Not Assigned")) 
				{
					//Not Assign Tasks
				}
				...
				if (newItem != null) {
					...
					if (searchBar.getText().equals("")) {
						...
						System.out.println(newItem.getTaskName());
							...
							|| newItem.getAssignDate().contains(searchBar.getText())
							|| newItem.getEstimateDate().contains(searchBar.getText())) {
